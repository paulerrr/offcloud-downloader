...
const log = debug('patbrid:watchers:offcloud');
import OffCloudAPI from './offcloudapi.js';
import OffCloudTorrent from './torrent.js';
import OffCloudQueueManager from './queuemanager.js';
...
class OffCloudWatcher {
  ...
  async checkWatchList() {
    log('checkWatchList', this.watchList.length);
  ...
  addToWatchList(torrent) {
    log('addToWatchList', torrent.file);
  ...
  removeFromWatchList(torrent) {
    log('removeFromWatchList', torrent.file);
...
export default OffCloudWatcher;