...
const debug = require('debug')('patbrid:watchers:offcloud:queue')
...
class OffCloudQueueManager {
  constructor(client, maxConcurrentDownloads = 3) {
    debug('Initializing queue manager')
    this.client = client
    this.queue = []
  ...
  async getFileHash(filePath) {
    try {
      ...
      console.log(`[!] Error generating file hash for ${filePath}: ${err.message}`);
      // Fall back to file size and modification time if can't read file
      try {
        const stats = await fs.promises.stat(filePath);
        return `${stats.size}-${stats.mtimeMs}`;
      } catch (statErr) {
        console.log(`[!] Error getting file stats: ${statErr.message}`);
  ...
  async isFileProcessed(filePath) {
    ...
    if (this.processedFiles.has(filePath)) {
      ...
      if (fileInfo.hash === currentHash && 
        ...
        console.log(`[!] File ${filePath} was already processed recently`);
        return true;
      }
    }
  ...
  async markFileAsProcessed(filePath) {
    try {
      ...
      console.log(`[+] Marked file ${filePath} as processed`);
      ...
      console.log(`[!] Error marking file as processed: ${err.message}`);
    }
  }
  ...
  async cleanupCompletedDownloads(maxAgeHours = 24) {
    debug('Running cleanup of completed downloads');
    ...
    try {
      ...
      if (!Array.isArray(history)) {
        console.log('[!] Invalid history format received from API');
        return;
      }
          ...
          if (ageHours > maxAgeHours) {
            try {
              ...
              while (!success && retryCount < 3) {
                try {
                  await this.client.delete(item.requestId);
                  success = true;
                } catch (retryErr) {
                  retryCount++;
                  const waitTime = Math.pow(2, retryCount) * 1000;
                  console.log(`[!] Retry ${retryCount}/3 for ${item.requestId} after ${waitTime}ms`);
              ...
              if (success) {
                ...
                console.log(`[+] Cleaned up old download: ${item.fileName} (${item.requestId})`);
              } else {
                retryErrors++;
                console.log(`[!] Failed to clean up ${item.requestId} after multiple retries`);
              }
            } catch (err) {
              console.log(`[!] Error cleaning up download ${item.requestId}: ${err.message}`);
              retryErrors++;
            }
          }
        }
      ...
      if (cleanedCount > 0) {
        console.log(`[+] Cleaned up ${cleanedCount} old downloads, freed ${(cleanedSpace / (1024 * 1024)).toFixed(2)}MB of space`);
        // Force storage check refresh
        this.lastStorageCheck = 0;
      } else if (retryErrors === 0) {
        console.log('[+] No old downloads to clean up');
      } else {
        console.log(`[!] Attempted to clean up downloads but encountered ${retryErrors} errors`);
      }
    } catch (err) {
  ...
  async addToQueue(file, processFunction) {
    ...
    try {
      ...
      try {
        ...
        console.log(`[!] Error reading file size: ${err.message}`)
      ...
      console.log(`[+] Added '${file}' to download queue (position: ${this.queue.length})`)
    ...
    try {
      ...
      while (this.queue.length > 0 && this.activeDownloads < this.maxConcurrentDownloads) {
        ...
        if (!this.hasEnoughStorage(nextItem.estimatedSize)) {
          console.log(`[!] Not enough storage available on Offcloud. Waiting for space to free up.`)
          console.log(`[!] Current queue length: ${this.queue.length} items, will retry later.`)
          
          // If we have a lot of items queued and not enough storage, try cleanup
          if (this.queue.length > 5) {
            console.log(`[!] Attempting to clean up old downloads to free space...`);
            await this.cleanupCompletedDownloads(12); // Try cleaning downloads older than 12 hours
            
            // Re-check storage
            await this.updateStorageFromHistory();
            if (this.hasEnoughStorage(nextItem.estimatedSize)) {
              console.log(`[+] Cleanup successful, continuing with queue processing`);
            } else {
        ...
        try {
          console.log(`[+] Processing queued file: ${nextItem.file} (${this.activeDownloads}/${this.maxConcurrentDownloads} active)`)
          await nextItem.processFunction(nextItem.file)
          this.removeFromQueue(nextItem)
        } catch (err) {
          console.log(`[!] Error processing ${nextItem.file}: ${err.message}`)
          nextItem.status = 'error'
          nextItem.lastError = err.message
          nextItem.retries++
          
          if (nextItem.retries >= nextItem.maxRetries) {
            console.log(`[!] Max retries reached for ${nextItem.file}, removing from queue`)
            this.removeFromQueue(nextItem)
          } else {
            // Put back in queue with lower priority
            nextItem.status = 'queued'
            nextItem.priority += 1
            console.log(`[+] Requeued ${nextItem.file} for retry (attempt ${nextItem.retries}/${nextItem.maxRetries})`)
            
            // Add exponential backoff
            const backoffMs = Math.pow(2, nextItem.retries) * 5000;
            console.log(`[+] Will retry after ${backoffMs/1000} seconds (backoff)`)
      ...
      console.log(`[!] Error processing queue: ${err.message}`)
    } finally {
      this.isProcessing = false
  ...
  removeFromQueue(item) {
    ...
    if (index !== -1) {
      ...
      console.log(`[+] Removed '${item.file}' from download queue`)
  ...
  async updateStorageFromHistory() {
    ...
    if (now - this.lastStorageCheck < 60000 && this.storageInfo) {
      return Promise.resolve(this.storageInfo)
    }
    ...
    try {
      ...
      const totalSpace = 1024 * 1024 * 1024 * 10; // Assume 10GB limit
      ...
      console.log(`[+] Storage info updated - Free: ${(this.storageInfo.freeSpace / (1024 * 1024)).toFixed(2)}MB, Used: ${(this.storageInfo.usedSpace / (1024 * 1024)).toFixed(2)}MB, Total: ${(this.storageInfo.totalSpace / (1024 * 1024)).toFixed(2)}MB`)
      ...
      if (this.storageInfo) {
        console.log(`[+] Using cached storage information from previous check`);
        return this.storageInfo;
      }
      ...
      this.storageInfo = {
        ...
        usedSpace: 0,
        freeSpace: 1024 * 1024 * 1024 * 10, // 10GB
        lastUpdated: now
      };
      
      console.log(`[+] Using default storage information due to error`);
      return this.storageInfo;
    }
  }
  ...
  hasEnoughStorage(estimatedSize) {
    ...
    if (!hasEnough) {
      console.log(`[!] Storage check failed - Need: ${(sizeWithBuffer / (1024 * 1024)).toFixed(2)}MB, Available: ${((this.storageInfo.freeSpace - this.minStorageRequired) / (1024 * 1024)).toFixed(2)}MB`);
    }
...
module.exports = OffCloudQueueManager