...
class OffCloudQueueManager {
  ...
  startPeriodicCleanup() {
    ...
    logger.debug('Scheduled periodic cleanup of completed downloads');
  }
            ...
            try {
              ...
              while (!success && retryCount < 3) {
                try {
                  ...
                  logger.warn(`Retry ${retryCount}/3 for ${item.requestId} after ${waitTime}ms`);
              ...
              if (success) {
                ...
                retryErrors++;
                logger.error(`Failed to clean up ${item.requestId} after multiple retries`);
              }
            } catch (err) {
